...
export function AssetOriginalInfo(props: AssetOriginalInfoProps) {
  ...
  const fromUrl = placeholderUrl ? (placeholderUrl.split("/").pop() || "") : "";
  ...
  return (
    <div className="grid grid-cols-[1fr_220px] gap-2 items-start">
      <div className="min-w-0 space-y-1">
        ...
        <div className="text-xs text-muted-foreground">大小: ≤ {maxFileSizeMb}MB</div>
      ...
      {assetType === "image" ? (
        <div className="relative w-[220px] aspect-[3/2] rounded-md overflow-hidden border bg-muted">
          {placeholderUrl ? (
            <Image src={placeholderUrl} alt={`${assetName}-原始`} fill className="object-cover" />
          ) : (
            <div className="w-full h-full grid place-items-center text-xs text-muted-foreground">原始</div>
          )}
        </div>
      ) : (
        <div className="w-[220px] h-[120px] rounded-md border bg-muted/50 grid place-items-center text-xs text-muted-foreground">
